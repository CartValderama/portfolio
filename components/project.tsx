...
export default function Project({
  ...
  return (
    <motion.div
      ...
      className="mb-3 sm:mb-8 last:mb-0 group bg-stone-100 border border-black/5 overflow-hidden relative rounded-lg"
    >
      <section className="relative flex group-even:flex-row-reverse">
        <div className="py-5 px-4 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col gap-y-10">
          ...
          <ul className="flex flex-wrap mt-4 gap-2">
            {tags.map((tag, index) => (
              <li
                className="bg-sky-900 px-3 py-1 text-[0.7rem] uppercase tracking-wider text-white rounded-full"
                key={index}
              >
                {tag}
              </li>
